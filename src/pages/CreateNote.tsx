...
import { 
  RiSparkling2Line, RiSaveLine, RiSendPlaneLine, RiPriceTag3Line, 
  RiFolderOpenLine, RiText, RiTimeLine, 
  RiMagicLine, RiTranslate2, RiCheckDoubleLine, RiUserLine,
  RiImageLine, RiLayoutGridLine, RiDatabaseLine
} from 'react-icons/ri';
...
export default function CreateNote() {
  const [formData, setFormData] = useState<NoteSchemaData>({
    ...
    coverImage: "/images/css-subgrid.png",
...
const [selectedAiFeature, setSelectedAiFeature] = useState<string | null>(null);
  ...
  const handleAuthorChange = (field: keyof NoteAuthor, value: string) => {
    setFormData(prev => ({
      ...prev,
  ...
  const executeSubmit = (targetStatus: 'Draft' | 'Published') => {
    setIsSaving(true);
    ...
    setTimeout(() => {
      ...
      setFormData(prev => ({
        ...
        status: targetStatus,
        updatedAt: todayStr,
        publishedAt: targetStatus === 'Published' ? todayStr : prev.publishedAt
      }));
      setLastSaved(`Saved successfully at ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`);
      alert("Payload submitted successfully to Database MongoDB!");
      console.log("Submitting Clean Note Schema Architecture:", {
        ...formData,
        status: targetStatus,
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 p-4 sm:p-6 lg:p-8 font-sans antialiased">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 mb-8 border-b border-slate-200 dark:border-slate-800">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Editing structural dataset for your full-stack notes architecture.</p>
        </div>

        <div className="flex items-center gap-3 self-end sm:self-auto">
          <button onClick={() => executeSubmit('Draft')} disabled={isSaving} className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium bg-white border border-slate-200 rounded-xl hover:bg-slate-50 dark:bg-slate-900 dark:border-slate-800 dark:hover:bg-slate-800 disabled:opacity-50 transition-all shadow-sm">
            <RiSaveLine className="w-4 h-4" />
            <span>Save Draft</span>
          </button>
          
          <button onClick={() => executeSubmit('Published')} disabled={isSaving} className={`flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r ${formData.coverGradient} rounded-xl opacity-90 hover:opacity-100 active:scale-[0.98] disabled:opacity-50 transition-all shadow-md`}>
        ...
        <main className="lg:col-span-7 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 p-6 shadow-sm space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between py-1.5 px-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-850 text-slate-500 text-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-slate-150 dark:border-slate-850">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  {/* Clean custom indicator chevron arrow since Tailwind resets native indicators */}
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-xl focus:outline-none font-mono text-xs"
                />
              </div>
            ...
            {formData.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 p-2 bg-slate-50 dark:bg-slate-950 rounded-xl border border-indigo-50/10">
                {formData.tags.map((tag, idx) => (
                  <span key={idx} className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-indigo-600 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-950/40 rounded-lg border border-indigo-100/50 dark:border-indigo-900/30">
                    {tag}
                    <button type="button" onClick={() => handleInputChange('tags', formData.tags.filter((_, i) => i !== idx))} className="hover:text-red-500 ml-1 font-bold">&times;</button>
                  </span>
                ))}
              </div>
            )}
...
<aside className="lg:col-span-3 space-y-4 lg:sticky lg:top-6">
  ...
  <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
    <div className="flex items-center gap-2 pb-2 border-b border-slate-100 dark:border-slate-850">
      ...
      <h2 className="text-xs font-bold uppercase tracking-wider text-slate-400">Author Meta Schema</h2>
    </div>
    
    <div className="space-y-3">
      <div className="space-y-1">
        <label className="text-[11px] font-medium text-slate-400">Author Name</label>
        <input 
          type="text" 
          value={formData.author.name}
          onChange={e => handleAuthorChange('name', e.target.value)}
          className="w-full px-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-lg focus:outline-none"
        />
      </div>

      <div className="space-y-1">
        <label className="text-[11px] font-medium text-slate-400">Avatar Image Path</label>
        <input 
          type="text" 
          value={formData.author.avatar}
          onChange={e => handleAuthorChange('avatar', e.target.value)}
          className="w-full px-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-lg focus:outline-none font-mono"
        />
      </div>

      <div className="space-y-1">
        <label className="text-[11px] font-medium text-slate-400">Author Bio Statement</label>
        <textarea 
          value={formData.author.bio}
          onChange={e => handleAuthorChange('bio', e.target.value)}
          rows={2}
          className="w-full px-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-lg focus:outline-none resize-none"
        />
      </div>
    </div>
  </div>

  {/* FULLY INTERACTIVE GEMINI CORE CONTEXT */}
  <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
    <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-850">
      <div className="flex items-center gap-2">
        <RiSparkling2Line className="w-4 h-4 text-indigo-500 animate-pulse" />
        <h2 className="text-sm font-semibold bg-gradient-to-r from-indigo-600 to-purple-500 dark:from-indigo-400 dark:to-purple-300 bg-clip-text text-transparent">Gemini Core Context</h2>
      </div>
      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/30">Sync</span>
    </div>
    ...
    <div className="space-y-2">
      {[
        ...
        // State check to see if this particular option is active
          ...
          <button 
            ...
            className={`w-full flex items-center justify-between p-2.5 text-left text-xs font-medium rounded-xl border transition-all ${
              isSelected 
                ...
                : 'border-slate-150 dark:border-slate-850 text-slate-700 dark:text-slate-300 bg-slate-50/50 dark:bg-slate-950/40 hover:bg-slate-100/50 dark:hover:bg-slate-800/40'
            }`}
          >
    ...
    <div className="pt-2 border-t border-slate-100 dark:border-slate-850 relative flex items-center">
      <input 
        type="text" 
        placeholder={selectedAiFeature ? `Instruct context for ${selectedAiFeature}...` : "Instruct companion prompt..."} 
      ...
      />
      <button type="button" className="absolute right-1.5 p-1.5 rounded-lg text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 transition-colors">
      ...
      <footer className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 py-4 px-6 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl shadow-xs text-xs text-slate-500 dark:text-slate-400 font-mono">
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
          <div>WORDS: <span className="font-semibold text-slate-700 dark:text-slate-300">{metrics.wordCount}</span></div>
          <div>CHARS: <span className="font-semibold text-slate-700 dark:text-slate-300">{metrics.charCount}</span></div>
          <div>STATUS: <span className="font-semibold px-2 py-0.5 rounded bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300">{formData.status}</span></div>
          <div className="flex items-center gap-1 text-slate-400"><RiTimeLine className="w-3.5 h-3.5" /> <span>UPDATED: {formData.updatedAt}</span></div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-950 py-1 px-2.5 rounded-lg border border-slate-100 dark:border-slate-850">
          ...
          <span className="text-[11px] text-slate-600 dark:text-slate-400">{isSaving ? 'Syncing active data model...' : lastSaved}</span>
        </div>
      </footer>
