...
const parseFavoriteDocument = (
  ...
  return {
    ...
    note: {
      ...
      coverGradient: String(
        core.coverGradient || "from-indigo-500 to-purple-600",
      ),
      ...
      author: {
        id: author.id,
        name: String(author.name || author.email || "Unknown"),
...
export default function FavoritesPage() {
    ...
    return favorites
      .filter(
        ({ note }) =>
          ...
          (categoryFilter === "all" ||
            note.category.toLowerCase() === categoryFilter.toLowerCase()),
      )
      .sort((a, b) => {
        if (sortBy === "title") return a.note.title.localeCompare(b.note.title);
        if (sortBy === "time")
          return (
            (parseInt(b.note.readTime) || 0) - (parseInt(a.note.readTime) || 0)
          );
        return 0;
      });
  ...
  const selectedDeleteNoteTitle = useMemo(() => {
    return (
      favorites.find((f) => f.note._id === deletingId)?.note.title ||
      "this note"
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 p-4 sm:p-8 max-w-7xl mx-auto transition-colors duration-200">
      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 mb-8 border-b border-slate-200 dark:border-slate-800">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">
            Quick access to your curated thoughts.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {[
            ...
            <div
              key={idx}
              className="relative flex items-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs"
            >
              ...
              <select
                ...
                className="pl-9 pr-8 py-2 text-sm appearance-none bg-transparent cursor-pointer rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              >
          ...
          <div className="hidden sm:flex p-1 bg-slate-200/60 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
            {(["grid", "list"] as const).map((mode) => (
              <button
                ...
                className={`p-1.5 rounded-lg transition-all ${viewMode === mode ? "bg-white dark:bg-slate-800 shadow-xs text-indigo-500" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"}`}
              >
                {mode === "grid" ? (
      ...
      {error && (
        <div className="mb-6 p-4 rounded-xl text-sm bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900/30">
          {error}
        </div>
      )}

      {loading ? (
        <div className="flex justify-center items-center py-20 text-slate-400 gap-2 animate-pulse">
          <RiSparkling2Line className="animate-spin" /> Loading your digital
          space...
        </div>
      ) : favorites.length > 0 ? (
        <>
          ...
          <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              ...
              {
                label: "Categories",
                val: new Set(favorites.map((f) => f.note.category)).size,
                sub: "Unique topics",
              },
              {
                label: "Filtered",
                val: filteredFavorites.length,
                sub: "Matches active filters",
                highlight: true,
              },
              {
                label: "Total tags",
                val: favorites.reduce((acc, f) => acc + f.note.tags.length, 0),
                sub: "Indexed words",
              },
            ].map((stat, i) => (
              <div
                key={i}
                className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-xs"
              >
                <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block">
                  {stat.label}
                </span>
                <div className="text-2xl font-bold mt-1 flex items-baseline gap-1.5">
                  <span className={stat.highlight ? "text-indigo-500" : ""}>
                    {stat.val}
                  </span>
          ...
          {filteredFavorites.length > 0 ? (
            ...
            >
              {filteredFavorites.map(({ _id, note }) => (
                <article
                  key={_id}
                  className={`group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xs hover:shadow-md transition-all duration-300 flex ${viewMode === "grid" ? "flex-col rounded-3xl overflow-hidden hover:-translate-y-1" : "flex-row items-center p-4 rounded-2xl gap-4"}`}
                >
                  {/* Card Visual Header */}
                  <div
                    className={`relative overflow-hidden bg-slate-100 dark:bg-slate-950 shrink-0 ${viewMode === "grid" ? "h-44 w-full" : "h-24 w-24 rounded-xl"}`}
                  >
                    {note.coverImage ? (
                      <img
                        src={note.coverImage}
                      ...
                      <div
                        className={`w-full h-full bg-gradient-to-tr ${note.coverGradient}`}
                      />
                    )}
                    {viewMode === "grid" && (
                  ...
                  {/* Card Content body */}
                  <div
                    className={`flex-1 flex flex-col justify-between ${viewMode === "grid" ? "p-5 space-y-4" : "h-full"}`}
                  >
                    <div>
                      <div className="flex justify-between items-start gap-2">
                        <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 group-hover:text-indigo-500 transition-colors line-clamp-1">
                          {note.title}
                        </h2>
                        {viewMode === "list" && (
                      ...
                      <p
                        className={`text-sm text-slate-500 dark:text-slate-400 mt-1 line-clamp-2 ${viewMode === "grid" ? "line-clamp-3" : "hidden sm:line-clamp-2"}`}
                      >
                        {note.description}
                      </p>
                    </div>
                      ...
                      <div className="flex items-center justify-between text-xs text-slate-400 border-t border-slate-100 dark:border-slate-800/60 pt-3">
                        <div className="flex items-center gap-3">
                          <span className="flex items-center gap-1">
                            <RiTimeLine /> {note.readTime}
                          </span>
                          <span className="flex items-center gap-1">
                            <RiEyeLine /> {note.views}
                          </span>
                        </div>
            ...
            <div className="flex flex-col items-center justify-center text-center p-12 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 min-h-[300px]">
              ...
              <p className="text-sm text-slate-400 max-w-xs mt-1 mb-4">
                No starred documents match your selected search queries or
                filter attributes.
              </p>
              <button
                onClick={() => {
                ...
                className="px-4 py-2 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors rounded-xl shadow-xs"
              >
                Clear Filter Rules
              </button>
            </div>
        ...
        <div className="flex flex-col items-center justify-center text-center p-12 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 min-h-[400px] shadow-2xs">
          ...
          <p className="text-sm text-slate-400 max-w-xs mt-1 mb-6">
            Star your important workspace notes to pin them instantly inside
            this dashboard.
          </p>
          <a
            href="/browse"
            className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 rounded-xl shadow-md shadow-indigo-500/10"
          >
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  ...
                  <span className="font-semibold text-slate-800 dark:text-slate-200">
                    "{selectedDeleteNoteTitle}"
                  </span>{" "}
                  from your favorites shelf?
                </p>
              </div>
            </div>
