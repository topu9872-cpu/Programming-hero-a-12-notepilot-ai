...
const getIconForCategory = (category: string) => {
  switch (category?.toLowerCase()) {
    case 'programming': return <Code className="w-4 h-4 text-indigo-500" />;
    case 'design': return <Palette className="w-4 h-4 text-pink-500" />;
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="leading-tight">
                          <p className="text-[11px] font-bold flex items-center gap-0.5 text-slate-800 dark:text-zinc-200">
                            <span className="truncate max-w-[70px]">{note.author?.name?.split(' ')[0] || 'User'}</span>
                      ...
                      <div className="flex items-center gap-2 text-slate-400 dark:text-zinc-500 text-[10px] font-bold">
                        <span className="flex items-center gap-0.5">
                          <Eye className="w-3 h-3" /> {note.views >= 1000 ? `${(note.views/1000).toFixed(0)}k` : note.views || 0}
                        </span>
                        <span className="flex items-center gap-0.5">
            ...
            {uniqueCategories.map((cat) => (
              ...
              >
                <div className="flex items-center gap-2.5">
                  <div className={`p-2 rounded-lg border bg-gradient-to-br ${cat.gradient} group-hover:scale-105 transition-transform`}>
                    {cat.icon}
                  </div>
                  <div>
