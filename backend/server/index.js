...
const AllNotesCollection = db.collection("All_Notes");
const FavoritedCollection = db.collection("Favorited");
async function startServer() {
    try {
        ...
        app.patch("/all-notes/:id", async (req, res) => {
            const { id } = req.params;
            const body = req.body;
            const result = await AllNotesCollection.updateOne({ _id: new ObjectId(id) }, {
                $set: body,
            });
            res.json(result);
        });
